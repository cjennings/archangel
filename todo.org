...
Current format: archangel-vmlinuz-6.12.66-lts-2026-01-24-x86_64.iso
New format: archangel-2026-01-24-vmlinuz-6.12.66-lts-x86_64.iso
...
** TODO [#A] Fix mkinitcpio configuration in archangel (causes boot failure)
After kernel updates or mkinitcpio regeneration, systems fail to boot because archangel
...
- Line 19: "EFI Boot Redundancy - GRUB installed on all disks" - now uses ZFSBootMenu for ZFS insalls
...
** TODO [#B] grub menu too small on hidpi displays
...
- make deploy - Copy ISO to truenas.local and USB drive (if present)
