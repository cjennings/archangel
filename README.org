...
- Build log: ~out/<iso-basename>.log~ on success (matches the ISO name), or
  ~out/build-YYYY-MM-DD-HHMM.log~ if the build failed before producing an ISO.
  Logs survive the ~work/~ cleanup and are tee'd from ~mkarchiso~ in real time.
